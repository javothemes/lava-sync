...
class AdminSetup
{
    ...
    private function handleGET()
    {
        ...
        <div class="wrap wpsynchro-setup">
            ...
            <p><?php _e('Configure the access key used for accessing this installation from remote. Treat the access key like a password and keep it safe from others.<br>This is also where you choose what methods (pull/push) are allowed when accessing this installation from remote.', 'moon114textdomain')domain'); ?></p>
            ...
            if ($this->show_update_settings_notice) {
                ...
                if (count($this->notices) > 0) {
                    ...
                    <div class="notice notice-success">
                        <p><?php _e('WP Synchro settings are now updated', 'moon114textdomain')domain'); ?></p>
                    </div>
                    <?php
                }
            }
            ...
            <form id="wpsynchro-setup-form" method="POST" >
                <div class="sectionheader"><span class="dashicons dashicons-admin-tools"></span> <?php _e('Configure settings', 'moon114textdomain')domain'); ?></div>
                <table class="">
                    <tr>
                        <td><label for="name"><?php _e('Access key', 'moon114textdomain')domain'); ?></label></td>
                        <td>
                            <input type="text" name="accesskey" id="wp_synchro_accesskey" value="<?php echo $accesskey; ?>" class="regular-text ltr" readonly><br>
                            <button id="generate_new_access_key" class="wpsynchrobutton"><?php _e('Generate new access key', 'moon114textdomain')domain'); ?></button>
                        </td>
                    </tr>
                    <tr><td><p></p></td><td></td></tr>
                    <tr>
                        <td><?php _e('Allowed methods', 'moon114textdomain')domain'); ?></td>
                        <td>
                            <label><input type="checkbox" name="allow_pull" id="allow_pull" <?php echo ( $methodsallowed->pull ? ' checked ' : '' ); ?>  /> <?php _e('Allow pull - Allow this site to be downloaded', 'moon114textdomain')domain'); ?></label><br>
                            <label><input type="checkbox" name="allow_push" id="allow_push" <?php echo ( $methodsallowed->push ? ' checked ' : '' ); ?> /> <?php _e('Allow push - Allow this site to be overwritten', 'moon114textdomain')domain'); ?></label>
                        </td>
                    </tr>
                    <tr><td><p></p></td><td></td></tr>
                    <tr>
                        <td><?php _e('Optimize compatibility', 'moon114textdomain')domain'); ?></td>
                        <td>
                            <label><input type="checkbox" name="enable_muplugin" id="enable_muplugin" <?php echo ( $enable_muplugin ? ' checked ' : '' ); ?>  /> <?php _e('Enable MU Plugin to optimize compatibility on WP Synchro requests (recommended)', 'moon114textdomain')domain'); ?> </label><br>                         
                        </td>
                    </tr>
                    <tr><td><p></p></td><td></td></tr>
                    <tr>
                        <td><?php _e('Debug log', 'moon114textdomain')domain'); ?></td>
                        <td>
                            <label><input type="checkbox" name="enable_debuglogging" id="enable_debuglogging" <?php echo ( $enable_debuglogging ? ' checked ' : '' ); ?>  /> <?php _e('Enable debug level logging - Generates much larger log files, but needed for debugging', 'moon114textdomain')domain'); ?></label><br>                         
                        </td>
                    </tr>
                    <tr><td><p></p></td><td></td></tr>
                    <tr>
                        <td><?php _e('IP security check', 'moon114textdomain')domain'); ?></td>
                        <td>
                            <label><input type="checkbox" name="enable_ip_check" id="enable_ip_check" <?php echo ( $enable_ip_check ? ' checked ' : '' ); ?>  /> <?php _e('Enable IP address security check during synchronization', 'moon114textdomain')domain'); ?></label>
                            <span title="<?= __('Enable the IP Security check during synchronization to decrease possibility of man-in-the-middle attacks. It may give problems on some sites, mostly when using multiple servers or routing through services like CloudFlare.', 'moon114textdomain')domain') ?>" class="dashicons dashicons-editor-help"></span><br>                         
                        </td>
                    </tr>

                </table>
                <p><input type="submit" value="<?php _e('Save settings', 'moon114textdomain')domain'); ?>" /></p>
