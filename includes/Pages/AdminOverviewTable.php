...
class AdminOverviewTable extends \WP_List_Table
{
    ...
    function column_name($item)
    {
        ...
        $actions['delete'] = sprintf("<a href='%s' onclick='return confirm(\"" . __('%s', 'moon114textdomain')domain') . "\"); '>%s</a>", $deletelink, __("Are you sure you want to delete this?", "wpsynchro"), __("Delete", "wpsynchro"));
    ...
    function column_location($item)
    {
        ...
        return __('Local installation', 'moon114textdomain')domain');
    }
    ...
    function column_synchronizes($item)
    {
        ...
        if ($item->sync_database) {
            $synchronized_text .= __('Database', 'moon114textdomain')domain');
        }
        if ($item->sync_files) {
            $synchronized_text .= __('Files', 'moon114textdomain')domain');
        }
    ...
    function column_actions($item)
    {
        ...
        if ($item->canRun()) {
            ...
            $actions .= "<a class='button runsyncjob' href='" . $runlink . "'>" . __('Run now', 'moon114textdomain')domain') . '</a>';
        } else {
            $actions .= "<a class='button runsyncjob' style='cursor:not-allowed;' title='" . __('Installation can not be run - See description', 'moon114textdomain')domain') . "' href='#' disabled>" . __('Ru'moon114textdomain')n114textdomain') . '</a>';
        }
    ...
    {
        $columns = array(
            'name' => __('Name', 'moon114textdomain')domain'),
            'type' => __('Type', 'moon114textdomain')domain'),
            'description' => __('Description', 'moon114textdomain')domain'),
            'actions' => __('Actions', 'moon114textdomain')domain'),
        );
        return $columns;
    }
    ...
    function get_views()
    {
        ...
        $views['all'] = "<a href='{$all_url }' {$class} >" . __('ALL TYPES', 'moon114textdomain')domain') . '</a>';
