...
class SynchronizeStatus
{
    ...
    public function setupStages()
    {
        ...
        if (isset($this->installation->sync_files) && $this->installation->sync_files) {
            $this->stages[] = $this->createSynchronizationStage("filessync", __('Synchronize files', 'moon114textdomain')domain'), __("Synchronize the files, by comparing and transferring the missing files", "wpsynchro"));
        }

        // Finalize
        $this->stages[] = $this->createSynchronizationStage("finalize", __('Finalizing', 'moon114textdomain')domain'), __("Completes the synchronization by doing the last few steps and cleaning up", "wpsynchro"));
    }
